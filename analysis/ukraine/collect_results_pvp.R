...
verbalizers = c("alpha", "normal", "shuffle")
archs = c("lora")
pvps = c("prompt", "no")
...
dat = rbindlist(lapply(datasets, function(dataset){
  ...
  rbindlist(lapply(models, function(model){
    rbindlist(lapply(verbalizers, function(verbalizer){
          ...
          rbindlist(lapply(samplings, function(sampling){
            rbindlist(lapply(nshots, function(nshot){
              rbindlist(lapply(samples, function(sample_i){
                path_to_pred = file.path(dataset, "pvps", model, verbalizer, arch, pvp, sampling, nshot, sample_i)
...
fwrite(dat, file.path("analysis", "ukraine", "results_pvp.csv"))
